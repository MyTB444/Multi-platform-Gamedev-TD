...
public class TowerUpgradeTester : MonoBehaviour
{
    ...
    private void RefreshValidUpgrades()
    {
        ...
        {
            validUpgrades.Add(TowerUpgradeType.LowerCooldown);
    ...
    private void Update()
    {
        ...
        // Press E to enable upgrade
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (selectedTower != null && validUpgrades.Count > 0)
            {
                selectedTower.SetUpgrade(GetCurrentUpgrade(), true);
                Debug.Log($"ENABLED {GetCurrentUpgrade()} on {selectedTower.name}");
            }
        }
        
        // Press R to remove upgrade
        if (Input.GetKeyDown(KeyCode.R))
        {
            if (selectedTower != null && validUpgrades.Count > 0)
            {
                selectedTower.SetUpgrade(GetCurrentUpgrade(), false);
                Debug.Log($"DISABLED {GetCurrentUpgrade()} on {selectedTower.name}");
            }
        }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Box("Tower Upgrade Tester");
        ...
        GUILayout.Label("E: Enable upgrade");
        GUILayout.Label("R: Remove upgrade");
        ...
        for (int i = 0; i < validUpgrades.Count; i++)
        {
            ...
            GUILayout.Label($"{prefix}{validUpgrades[i]}");
        }
